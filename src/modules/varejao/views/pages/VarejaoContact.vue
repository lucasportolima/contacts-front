        ...
        <v-card class="table-page pt-7">
            <div class="table-page d-flex flex-column justify-space-between">
                <div>
                    ...
                    <template v-if="!varejaoContacts.length">
                        <div class="pt-10">
                            <data-not-found
                                v-if="!filterPristine"
                                title="Contato Varejao não encontrado."
                                :img-src="require('@/assets/Ativo1.svg')"
                            />

                            <data-not-found
                                v-if="filterPristine"
                                title="Você ainda não cadastrou nenhum contato Varejao, deseja cadastrar?"
                                btn-label="Cadastrar o primeiro contato Varejao"
                                :btn-action="goToAddContact"
                                :img-src="require('@/assets/Ativo1.svg')"
                            />
                        </div>
                ...
                <div v-if="varejaoContacts.length">
...
<script>
    ...
    import VarejaoMenuItem from "@/modules/varejao/views/components/VarejaoMenuItem";
    ...
    export default {
        ...
        methods: {
            ...mapActions("Varejao", ["getContactsVarejao"]),
            ...mapActions(["toggleSnackbar"]),

            goToAddContact() {
                this.$router.push({ name: "NewVarejaoContact" });
            },

            async fetchVarejao() {
                try {
                    const payload = this.getPayloadData();
                    const { count, results } = await this.getContactsVarejao(
                        payload
                    );
                    this.varejaoContacts = results;
                    this.setTableInfo({
                        totalItems: count,
                        totalPages:
                            count / 10 < 1
